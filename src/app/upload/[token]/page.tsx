...
/** Fetch the request row securely on the server */
async function getRequest(token: string) {
  ...
  const okStatus = ['pending','emailed','uploaded','processing'].includes(data.status || '');
  if (!okStatus || Date.now() > +expires) return null;
  return data;
}
    ...
    <main className="mx-auto max-w-lg p-6">
      ...
      >
        ...
        <div className="rounded border p-3">
          ...
          />
          <p className="text-xs text-slate-500 mt-1">Tips: 1–3 photos, stand in a corner, include floor + two walls.</p>
        </div>
