      ...
      {topRooms.map((room) => {
        ...
        return (
          <div key={room.room_name} className="flex items-center gap-2 text-xs">
            <div className="w-20 truncate text-slate-600">{room.display_name}</div>
            ...
            <div className="w-16 text-right tabular-nums text-slate-700">{formatCurrency(room.total_with_vat)}</div>
          </div>
        );
      })}
    </div>
...
export default function RoomCard({ room, allRooms = [], showCharts = false, pendingUploads = [] }: RoomCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-lg hover:border-slate-300 transition-all duration-300 group max-w-full">
      {/* Header */}
      <div className="p-4 pb-2">
        ...
        {room.floor && (
          <p className="text-sm text-slate-600 mt-0.5">{room.floor} Floor</p>
        )}
        {subtitle && (
      ...
      {/* Image Section */}
      <div className="relative w-full h-48 bg-slate-50 overflow-hidden px-4 rounded-lg">
        <img
        ...
        {hasImages && images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-6 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-slate-700 hover:bg-white transition-colors shadow-sm"
            ...
            <button
              onClick={nextImage}
              className="absolute right-6 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-slate-700 hover:bg-white transition-colors shadow-sm"
        ...
        {isZeroCost ? (
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 shadow-sm">
              <div className="text-sm font-medium text-slate-700 mb-1">Total Refurbishment Cost</div>
              <div className="text-2xl font-bold text-slate-900">
                {formatCurrency(room.total_with_vat)}
              </div>
              <div className="text-xs text-slate-600 mt-1">Including VAT</div>
